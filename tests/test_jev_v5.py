"""Behavioral v5 regression tests; no network or historical-version imports."""
import contextlib
import copy
import csv
import hashlib
...
import evaluate_jev_v5 as runner
import jev_v5 as current
...
class InferenceTests(unittest.TestCase):
    def test_original_v5_requests_and_predictions_are_unchanged(self):
        reference = json.loads(Path(__file__).with_name("jev_v5_reference.json").read_text(encoding="utf-8"))
        def digest(value):
    ...
    def test_retention_boundaries_invalid_confidence_and_choice_priority(self):
        ...
        for value, recall, reduce in ((0, 6, 6), (.2999, 6, 6), (.3, 4, 5), (.6999, 4, 5), (.7, 3, 3), (1, 3, 3),
...
class RunnerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root = parent / (".tmp_v5_" + uuid.uuid4().hex)
        self.root.mkdir()
        def cleanup():
            assert self.root.resolve().parent == parent
            shutil.rmtree(self.root)
        self.addCleanup(cleanup)
        self.out = self.root / "results/jev_v5_mini"
    ...
    def test_full_run_resume_and_offline_verify_do_not_repeat_calls(self):
        ...
        self.assertEqual(frozen["inference"]["recall"]["medium_keep"], 4)
    ...
    def test_provenance_and_all_previous_outputs_protected(self):
        ...
        network.assert_not_called()
        for name in ("jev_v2_mini", "jev_v3_mini_reduce_origin", "jev_v4_mini"):
