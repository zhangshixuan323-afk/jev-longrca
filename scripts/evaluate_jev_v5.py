"""JEV Choice v5 Mini/Full runner with checkpoint replay and paired reporting."""
import argparse
import collections
...
import jev_v5 as inference

ROOT = baseline.ROOT
SOURCE_FILES = ("evaluate_jev_v5.py", "jev_v5.py", "evaluate.py", "evaluate_full.py", "resume_full.py")
...
def run_config(rows, manifest, subset, smoke, config):
    if inference.approved_prompts() != inference.PROMPTS or inference.PROMPTS != inference.expected_prompts():
        raise ValueError("v5 runtime prompt text differs from the approved exact intervention")
    return {"protocol": inference.PROTOCOL, "inference": config, "subset": subset, "smoke": smoke,
            ...
            "prompt_file": "prompts/jev_v5.json", "prompt_file_sha256": baseline.sha(inference.PROMPT_FILE.read_bytes()),
            ...
            "changes": {"all_prompts": "exact original v1 RULES plus root/role questions, no appendices",
                        "selection_history": "retain v4 input fields in reduction and final requests",
...
def prepare(out, config):
    target = out.resolve()
    for pattern in ("jev_v2*", "jev_v3*", "jev_v4*"):
        for path in (ROOT / "results").glob(pattern):
            if path.is_dir():
                source = path.resolve()
                if target == source or source in target.parents or target in source.parents:
                    raise ValueError("v5 must use an output directory separate from all earlier results")
    out = out.resolve()
    # Also protect ancestors/descendants so a typo cannot mix v5 with old outputs.
    for name in ("phase1", "full", "laya_full", "laya_smoke"):
        ...
        if out == old or old in out.parents or out in old.parents:
            raise ValueError("Choose a separate JEV v5 output directory")
    ...
    if path.exists():
        ...
        raise ValueError("Refusing a nonempty output directory without a matching v5 config")
...
def compare_reference(rows, predictions, directory, sample_hashes=None):
    ...
    return {"available": True, "directory": str(directory), "paired_n": len(matched),
            ...
            "details": details, "note": "Historical single-run comparison. Relative to v4 only prompt text changes; all stages rerun."}
...
def _write_summary(rows, predictions, manifest, out, config, subset, elapsed, stopped):
    ...
    summary["baseline_comparison"] = {"available": False, "note": "See v5 comparisons instead."}
...
def write_reports(rows, predictions, manifest, out, config, subset, elapsed, stopped):
    ...
    lines = ["# JEV v5：Mini/Full 完整流程评测", "",
             ...
             "v5 的全部步骤题和角色题逐字使用 v1 原文，包括共同 RULES 和独立角色归因要求。候选筛选及输入字段与 v4 相同：缩减高/中/低置信档为 3/5/6，初筛为 3/4/6，阈值仍为 0.30/0.70。", "",
    ...
    lines += ["", "## 用量和解释范围", "",
              ...
              "相对 v4，本轮只将全部提示词恢复为 v1 原文，输入字段和保留策略不变；相对最初 v1，保留策略和历史输入均不同。各阶段从头重跑，与历史运行比较仍包含服务时段和重复调用波动，不能将差异全部归因于某一句 prompt。", "",
              "[逐条结果](predictions.csv) · [配对比较](comparisons.json) · [指标](metrics.json) · [六份 v5 提示词](../../reports/jev_v5_prompts.md)", ""]
...
def main(argv=None):
    ...
    if inference.approved_prompts() != inference.PROMPTS:
        parser.error("Runtime prompts differ from the v5 approved Markdown")
    out = ROOT / (args.output or ("results/jev_v5_" + args.subset + ("_smoke" if args.smoke else "")))
